...
- [T099: quoted spans are sometimes data, sometimes code] — the quoted-span form of T095's defect C **blocks** commands (not just prompts): `grep -r "git push"` is refused while any task is In Progress. The obvious fix fails OPEN — a naive quoted-span strip turns `bash -c "git push"` and `ssh host "..."` into ALLOW, because unlike a heredoc body a quoted span can be executable code. Needs wrapper-aware classification. C2/Medium/P1, guide written. → tasks/TASK_GUIDE_T099.md
