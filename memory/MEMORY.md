...
- [T099 registered: quoted-span twin of T095 defect C] — `post_bash_memory_update.py` still reads `git push` inside a quoted argument as a real push (seen twice during T095's own Stage 5). Reuse `QUOTED_SPAN_PATTERN` + `lib/shell_data.py`; keep the strip-less-on-doubt rule. C1/Low/P2.
