...
- [CLAUDE.md gains context-overwhelm self-monitoring rule, 2026-08-03](decisions.md#claudemd-gains-a-context-overwhelm-self-monitoring-rule-2026-08-03) — Supervisor's own long-session accuracy, judgment-based off observed behavior, not a rigid trigger (user rejected that).
...
- [A test that shares a root can't detect a root-split defect](learnings.md#a-test-that-shares-a-root-cannot-detect-a-root-split-defect-2026-07-31-t047-stage-4) — T047's write/read paths collapse inside a worktree so the check can't see the split; straddle the boundary or it can't distinguish pass from fail.
...
- [A hook that blocks via stdout JSON fails OPEN when it raises](learnings.md#a-hook-that-blocks-via-stdout-json-fails-open-when-it-raises-t064-2026-08-09) — an uncaught import error exits non-blocking, so the merge proceeds; fail-closed must be actively emitted, never assumed. 4th way this gate has failed to gate.
- [A test can pin a section's *location*](learnings.md#a-test-can-pin-a-sections-location-and-a-move-shaped-task-cannot-pass-it-t064-2026-08-09) — two tests pinned the `verify` row to files T064 vacates; no impl satisfies both AC and test, only a test edit does. Stage 2 pre-flight for move-shaped tasks: grep the suite for vacated paths.
...
- [A scope guard committed as an invariant blocks what it guarded](learnings.md#a-scope-guard-committed-as-an-invariant-blocks-the-thing-it-was-guarding-t065-2026-08-09) — a pinned count/hash/line forbids the next legitimate edit; occurrences 4-6 hit T071 in one task (shape check + shared-constant split). Full writeup in learnings.md.
...
- [Volume is not cost when nearly everything is a cache read](learnings.md#volume-is-not-cost-when-nearly-everything-is-a-cache-read-2026-08-07-t061-investigation) — trimming injected context is worth ~a tenth of nominal; the ~15.7k-token floor per spawn is the real lever, not content. n=3, directional not established.
...
- [An empty Todo column can mean "never registered", not "nothing left"](learnings.md#an-empty-todo-column-can-mean-never-registered-not-nothing-left-2026-08-07) — T061's board looked finished at 0 Todo/59 Done, but the follow-on directions existed only as prose. Register selected items even when every one is blocked.
...
- [DDR-0004: Gate 1 upheld — cheaper spawns, not fewer](../docs/ddr/0004-uphold-hard-stop-gate-1-over-spawn-elimination.md) — user ruling 2026-08-07: spawn *count* is the cost lever, but eliminating spawns means the Supervisor implements, which Gate 1 forbids; exemption rejected. T062 closed unimplemented. Full reasoning in the DDR.
