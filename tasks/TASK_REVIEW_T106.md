...
| verify | ☑ pass | User-invoked `/verify` 2026-09-06, Supervisor-run **at the installer**, not the test suite. The README now asserts a fact about what `setup.sh` does, so that was the surface. Real Codex install from this branch into a throwaway git repo: the installer's own output named exactly the README's four skills with matching byte sizes and the skipped-not-truncated wording — `codex: skill 'bugfix' has a 10173-byte body, over the 8192-byte codex cap — SKIPPED, not truncated` (likewise `craft-spawn-prompt` 10109, `diagnose` 13548, `write-better-skill` 14568), then `4 skill(s) were SKIPPED for 'codex' ... They are absent, not truncated.` Checked the resulting filesystem rather than trusting the log: 26 skills in `.codex/skills/`, all four README names absent on disk. Three probes: (a) `HARNESS_SKILL_BODY_CAP=0` → 0 warnings and both oversize skills present, so the README's "currently affected" hedge is accurate and the set is not fixed; (b) a Claude install → 0 cap warnings and all 30 skills including both oversize ones, so the claim is correctly scoped to Codex; (c) the advertised site URL → HTTP 200, 31112 bytes. Verdict: **PASS**. |
