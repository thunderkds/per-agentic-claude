...
|-------|--------|------------------------|
| **New test(s) cover Acceptance Criteria (file paths pasted)** | ☑ pass | `tests/test_harness_projection.sh` — 41 assertions, **41 passed, 0 failed**. One test per defect (A: 4 assertions; B: AC2) plus AC1, AC4, AC5 anti-vacuity, AC6, AC7, AC8, AC9, AC10. Wired into `.github/workflows/ci.yml` (run + shellcheck) — it was not previously reached by any CI step. |
...
| verify | ☑ pass | **Stage 5 `/verify` PASS — 2026-08-31, user-invoked.** Driven at the real surface, not the suite: the real `setup.sh`/`update.sh` run against scratch git repos from a `file://` clone of the branch, plus a real Codex 0.149.1 session. 9 steps, 6 of them adversarial probes. Codex **executed** `wake` (not merely listed it) and reported `diagnose` (over-cap) unavailable — AC3 reproduced independently of the implementer's transcript. Probes: `--harness banana`, bare `--harness`, `--harness=`, `HARNESS_SKILL_BODY_CAP=abc`, double-run idempotency + planted orphan, no-flag `update.sh` re-projection, both-harness and default installs. Two findings, neither blocking; the first is registered as **T098**. Full report in the session transcript. |
