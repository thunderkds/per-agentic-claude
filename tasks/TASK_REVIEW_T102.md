...
| verify | ☐ N/A | Stage 5 `/verify` is user-run only (per `memory/MEMORY.md`: "verify skill is user-only") — not run by this agent |
...
1 failed in 0.02s
```
This is the control that mattered: the first draft of the new test used `os.path.basename(AGENTS_DIR)`, which collapses `.claude/agents` and `agents` to the same string `agents/` — M3 initially came back GREEN, proving the assertion was reading a literal, not the source. Fixed to `os.path.relpath(AGENTS_DIR, ROOT)` so the full path segment is compared; M3 was re-run and confirmed RED as required. Reverted `AGENTS_DIR` to `os.path.join(ROOT, "agents")`; full `test_site_content.py` suite green again (21 passed).
