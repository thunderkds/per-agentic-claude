...
| verify | ☑ pass | **Supervisor-run `/verify`, two rounds — round 2 is a pass.** Driven at the hooks' real surface: event JSON piped to both hook executables with `CLAUDE_PROJECT_DIR` set and an armed board, comparing a pre-fix root at `ced5ddd` against the branch. Not an import-and-call. <br><br>**Round 1 (`cdd9e6b`) FAILED.** The AC1-AC7 matrix passed, but probing wrappers outside `WRAPPER_PATTERN` found it was an allowlist of spans to *keep*, so an unlisted wrapper stripped the span and the push inside went invisible: `eval "<push>"`, `su user -c "<push>"` and `perl -e "system('<push>')"` each went BEFORE=BLOCK -> AFTER=allow. Fail-open, the exact direction the task exists to prevent, and AC8's probes could not see it (they substitute the naive strip, a different failure than an incomplete allowlist). <br><br>**Round 2 (`2b35cfa`) PASS.** The default was inverted to keep-unless-a-data-command-precedes. Re-measured: all three round-1 regressions BLOCK; the full AC1-AC7 matrix unchanged with no collateral movement; five unlisted executors block including an **invented** one (`zorblax --run`), which is the load-bearing evidence that the default now closes by construction rather than by list coverage; five `python -c` shell-reaching spellings block (`os.system`, `subprocess.run`, `os.popen`, `check_output`, `__import__('os').system`); `printf`/`rg`/`grep -n` still allow. <br><br>**One residual, accepted and disclosed by the implementer before it was found:** `python3 -c "import os as o; f=o.system; f('<push>')"` -> allow. Aliasing dodges `SPAN_EXECUTOR_PATTERN`'s literal spellings. Accepted because evading it requires deliberately obfuscating a push — a different threat model from the accidental blocking this gate exists to catch — and closing it means either dropping `python -c` from the data list (an AC1 change) or parsing Python (the rabbit hole `shell_data.py` exists to avoid). Recorded rather than rounded up to closed. — **pass** |
...
round-2 AFTER 04:28Z after a failed Stage 5 `verify`), under
