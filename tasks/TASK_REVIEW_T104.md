...
| verify | ☐ N/A | Stage 5 `verify` is user-only per project memory (`project_verify_skill_is_user_only.md`) — not run by this agent; Supervisor must ask the user to run `/verify` before merge — N/A for this agent's scope. |
